...
int main ( int argc, char** argv )
{
    ...
    cout<<"tamaño imagen : "<< src.rows<< " , "<<src.cols<<endl<< "total pixeles : "<<src.rows * src.cols<<endl;
    ...
    // src - (blanco-dest)
    imwrite( "resultado.jpg" , src-hola );
...
vector<Punto> intensidadColores( Mat &res )
{
    ...
    for(  ; it != end; ++it) // contar aparicion de cada tonalidad
    {
        ...
        medio = (*it)[0] == (max0+min0)/2 || (*it)[1] == (max1+min1)/2 || (*it)[2] == (max2+min2)/2;
        if ( max || min || medio)
        {
...
vector<Punto> manual(Mat & img)
{
    cout<<endl<<"Seleccione con el mouse areas en la imagen para definir los pixeles semilla"<<endl;
    immg = img.clone();
    namedWindow( "image", 1 );
